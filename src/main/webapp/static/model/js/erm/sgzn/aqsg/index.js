...
var d;
$(function(){
	dg=$('#sgzn_aqsg_dg').datagrid({    
	method: "post",
    url:ctx+'/sgzn/aqsg/list', 
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
...
//弹窗增加
function add(u) {
	openDialog("添加安全事故信息",ctx+"/sgzn/aqsg/create/","1000px", "600px","");
}
...
function upd(){
	...
	openDialog("修改安全事故信息",ctx+"/sgzn/aqsg/update/"+row.id,"1000px", "600px","");
...
function view(){
	...
	openDialogView("查看安全事故信息",ctx+"/sgzn/aqsg/view/"+row.id,"1000px", "600px","");
...
function fileexportword(){
	...
	$.ajax({
		url:ctx+"/sgzn/aqsg/exportword/"+row.id,
		type:"POST",
		success:function(data){
