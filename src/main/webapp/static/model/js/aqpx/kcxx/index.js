...
//弹窗增加
function add(u) {
	openDialog("添加课程信息",ctx+"/aqpx/kcxx/create/","800px", "400px","");
}
...
function upd(){
	...
	openDialog("修改课程信息",ctx+"/aqpx/kcxx/update/"+row.id,"800px", "400px","");
...
function openkjlist() {
	layer.open({
	    ...
	    title: '选择课件',
        maxmin: false, 
	    content: ctx + "/aqpx/kjkgl/kjchoose?type=priv" ,
	    btn: ['确定', '关闭'],
...
//弹出检查点选择框
function openPubList() {
	layer.open({
	    type: 2,
	    area: ['700px', '95%'],
	    title: '选择课件',
        maxmin: false,
	    content: ctx + "/aqpx/kjkgl/kjchoose?type=pub" ,
	    btn: ['确定', '关闭'],
	    yes: function(index, layero){
	    	var  $list= $("#jcdList");
	    	$list.html("");
