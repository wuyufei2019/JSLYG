// 弹窗增加
function add(u) {
	openDialog("添加风险点信息", ctx + "/fxgk/fxxx/create/", "100%", "100%", "");
}
...
function addfz() {
	...
	openDialog("复制风险点信息", ctx + "/fxgk/fxxx/addfz/" + row.id, "100%","100%", "");
}
...
function upd() {
	...
	openDialog("修改风险点信息", ctx + "/fxgk/fxxx/update/" + row.id, "100%","100%", "");
}
...
function view() {
	...
	openDialogView("查看风险点信息", ctx + "/fxgk/fxxx/view/" + row.id, "100%","100%", "");
}

//查看风险点巡检记录
function viewXjjl(fxdid) {
	openDialogView("查看历史巡检记录", ctx + "/fxgk/fxxx/viewXjjl/" + fxdid, "900px","400px", "");
}
...
//跳转统计分析
function tjfx() {
    openDialogView("查看统计分析", ctx+'/fxgk/tjfx/index', "100%","100%", "");
	//window.location.href = ctx+'/fxgk/tjfx/index';
}

//跳转风险点分布图
function fxdfbt() {
    openDialogView("查看风险点分布图", ctx+'/fxgk/fxyt/map', "100%","100%", "");
...
function exportexc(){
	...
	layer.open({
      ...
      maxmin: false, 
      shift: 1,
	    content: ctx+'/fxgk/fxxx/colindex',
		btn: ['导出'],
...
function exportbs() {
	...
	$.ajax({
		url : ctx + "/fxgk/fxxx/exportbs/" + row.id,
		type : "POST",
		success : function(data) {
...
function exportfxgz(type){
	...
	$.post(ctx + "/fxgk/fxxx/exportfxgz/" + row.id,{"type":type},function(data){
...
function exportpdf() {
	...
	$.ajax({
		url : ctx + "/fxgk/fxxx/exportpdf/" + row.id,
		type : "POST",
		success : function(data) {
			window.open(ctx + data);
		}
...
function checkContentManage(){
	...
	openDialogView("巡检内容管理",ctx+"/fxgk/fxxx/bdxjnr/"+row.id+"?qyid="+row.id1,"100%", "100%","");
}