...
function createInfoWindow2(json) {
	var iw = new BMap.InfoWindow('<div style="width:280px;height:40px; cursor: pointer;" onclick="Show' + json.type + '(' + json.id + ')" ><p style="font-size:14px;">名称：' + json.title + '</br>地址：' + json.address + '</br></p></div>');
	return iw;
}
...
// 创建marker
window.addMarker2 = function(data) {
	for (var i = 0; i < data.length; i++) {
		var json = data[i];// 整数据
		...
		(function() {
			var _json = json;
			var _iw = createInfoWindow2(_json);
			var _marker = marker;
			_marker.addEventListener("click", function() {
				this.openInfoWindow(_iw);
			});
