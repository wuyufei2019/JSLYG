...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            createLabel();
            if('${bro.mappoint}'){
                Global_Polygon = createPolygonSingle('${bro.mappoint}',"blue");
                if(Global_Polygon){
                    map.addOverlay(Global_Polygon); //增加多边形
                    Global_Polygon.enableEditing();
                    Global_Polygon.addEventListener("click", function() {
                        savePolygon();
                    });
                }
            }
        ...
        function createPolygon() {
            for(var i=0;i<wglist.length;i++){
                var json=wglist[i];
                var singlePolygon=createPolygonSingle(json.mappoint);
                if(singlePolygon){
                    map.addOverlay(singlePolygon); //增加多边形
                    (function() {
                        var wgname=json.m1;
                        var _polygon = singlePolygon;
                        PolygonAddEvent(_polygon,wgname);
                    })()
                }
            }
        ...
        //点击多边形弹出网格名称提示
        function PolygonAddEvent(singlePolygon,wgname){
            singlePolygon.addEventListener("click", function() {
                //this.openInfoWindow(_iw);
                layer.msg(wgname);
            });
        }
        //返回多边形对象
        function createPolygonSingle(mappoint,color){
            if(mappoint!=null&&mappoint!=""&&mappoint!="undefined"){
                ...
                return new BMap.Polygon(maparry,setting); //创建多边形
            }else return null;
        }
        ...
        function mapclear() {  //清除覆盖物
            map.removeOverlay(Global_Polygon);//移除多边形
