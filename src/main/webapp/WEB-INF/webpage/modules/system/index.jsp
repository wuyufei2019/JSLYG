...
<%
    String logotitle="连云区公共安全监管（应急指挥）云平台"; String logoimg="/static/model/main/images/logo.png";
...
<div id="container">
    ...
    <div id="navheader">
        ...
        <div class="topright">
            <ul>
                <li><a href="${ctxStatic}/app/JSLYG.apk" >下载APP</a></li>
                <li><a href="${ctx}/a/visualData" >可视化大数据</a></li>
                ...
                if(item.children && item.children.length>0) {
                    $.each(item.children, function (i, item) {
                    	if(item.children && item.children.length>0){
                    		...
                    		html += '<li id="cap" class="nav-li" '+(item.href ? 'href="${ctx}/' + item.href + '" data-id="' + item.id + '" ' : '')+'>';
                            html += '<i class="nav-icon-hide"></i><a href="javascript:;" class="ue-clear"><span class="nav-ivon"></span><span class="nav-text">' + item.name + '</span></a>';
                    	}
                        //三级菜单
                        if(item.children && item.children.length>0){
                            html+='<ul class="subnav">';
                            $.each(item.children, function (i, item) {
                                html+='<li class="subnav-li" '+(item.href ? 'href="${ctx}/' + item.href + '" data-id="' + item.id + '" ' : '')+'><a href="javascript:;" class="ue-clear"><i class="subNav-icon"></i><span class="subnav-text">'+item.name+'</span></a>';
                                if(item.children && item.children.length>0){
                                    html+='<ul class="three-nav">';
                                    //四级菜单
                                    $.each(item.children, function (i, item) {
                                        html+='<li '+(item.href ? 'href="${ctx}/' + item.href + '" data-id="' + item.id + '" ' : '')+' class="threenav-li"><a href="javascript:;">'+item.name+'</a></li>';
                                    });
                                    html+='</ul>';
                                }
                                html+='</li>';
