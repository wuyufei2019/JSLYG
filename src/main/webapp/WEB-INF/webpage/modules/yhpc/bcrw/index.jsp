    ...
    $(function () {
        dg = $('#yhpc_bcrw_dg').datagrid({
            ...
            onLoadSuccess: function () {
                dg.datagrid("autoMergeCellAndCells", ['depname']);
            },
    ...
    $(function () {
        dg2 = $('#yhpc_bcrw_dg2').datagrid({
            ...
            columns: [[
                {field: 'ID', title: 'id', checkbox: true, width: 50, align: 'center'},
                {field: 'name', title: '班次名称', sortable: false, width: 100},
                {field: 'm1', title: '巡检点名称', sortable: false, width: 100},
                {
                    field: 'timearea', title: '时间范围', sortable: false, width: 100,
                    formatter: function (value, row) {
                        return new Date(row.starttime).format("yyyy-MM-dd hh:mm:ss") +"-"+new Date(row.endtime).format("yyyy-MM-dd hh:mm:ss");
                    }
                },
                {
                    field: 'type', title: '班次类型', sortable: false, width: 100, align: 'center',
                ...
                {
                    field: 'status', title: '完成情况', sortable: false, width: 100, align: 'center',
                    formatter: function (value, row, index) {
                        if (value == 1)
                            return '待巡检';
                        else if(value ==2)
					...
					<span id="divper">
						<shiro:hasPermission name="yhpc:bcrw:add">
                            <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                    onclick="add()" title="添加"><i class="fa fa-plus"></i> 添加</button>
                        </shiro:hasPermission>
						<shiro:hasPermission name="yhpc:bcrw:update">
                            <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                    onclick="upd()" title="修改"><i class="fa fa-file-text-o"></i> 修改</button>
                        </shiro:hasPermission>
						<shiro:hasPermission name="yhpc:bcrw:delete">
                            <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                    onclick="del()" title="删除"><i class="fa fa-trash-o"></i> 删除</button>
                        </shiro:hasPermission>
						<shiro:hasPermission name="yhpc:bcrw:view">
                            <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                    onclick="view()" title="查看"><i class="fa fa-search-plus"></i> 查看</button>
                        </shiro:hasPermission>
			        		<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                    onclick="teamSetting()" title="班组设置"><i
                                    class="fa fa-bitbucket-square"></i>班组设置</button>
			        	</span>
                        <span id="divper2">
						</span>
                        <button class="btn btn-white btn-sm " data-toggle="tooltip" data-placement="left"
                ...
                <div class="col-sm-12">
                    <div class="pull-left">
                        <form id="searchFrom2" style="margin-bottom: 8px;" action="" class="form-inline">
                            <div class="form-group">
                                <input name="yhpc_bcrw_type2" class="easyui-combobox" style="height: 30px;"
                                       data-options="panelHeight:'auto',editable:false,prompt: '班次类型',data: [
													{value:'1',text:'日检'},
											        {value:'2',text:'周检'},
											        {value:'3',text:'月检'},
											        {value:'4',text:'年检'}]"/>
                                <span class="btn btn-primary btn-rounded btn-outline btn-sm " onclick="search2()"><i
                                        class="fa fa-search"></i> 查询</span>
                                <span class="btn btn-primary btn-rounded btn-outline btn-sm " onclick="reset2()"><i
                                        class="fa fa-refresh"></i> 全部</span>
                            </div>
                        </form>
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="pull-left">
                                <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left"
                                        onclick="view2()" title="查看"><i class="fa fa-search-plus"></i> 查看
                                </button>
                                <button class="btn btn-white btn-sm " data-toggle="tooltip" data-placement="left"
                                        onclick="refresh()" title="刷新"><i class="glyphicon glyphicon-repeat"></i> 刷新
                                </button>
...
<script>
    function teamSetting() {
        openDialog("排班设置", "${ctx}/yhpc/teamsetting/index", "100%", "100%");
    }
