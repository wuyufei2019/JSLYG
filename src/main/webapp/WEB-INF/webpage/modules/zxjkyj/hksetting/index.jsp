...
<script type="text/javascript">
    ...
    $(function () {
        ...
        dg = $('#dg').datagrid({
            nowrap: false,
            method: "post",
            url: '${ctx}/zxjkyj/hkvision/device-qy/list',
            fit: true,
            fitColumns: true,
            selectOnCheck: false,
            nowrap: false,
            ...
            columns: [[
                ...
                {field: 'validateCode', title: '设备验证码', sortable: false, width: 100, align: 'center'},
                {
                ...
                {
                    field: 'bind', title: '操作', sortable: false, width: 200, align: 'center'
                    , formatter: function (value, row) {
                        var html =
                            "<a class='btn btn-primary btn-xs' " +
                            "onclick='liveList(\"" + row.deviceSerial +"\",\""+row.qyname+ "\")' >直播列表</a>";
                        html += "<a class='btn btn-success btn-xs' " +
                            "onclick='bind(\"" + row.deviceSerial + "\",\"" + row.qyname + "\")'>开通直播</a>";
                        return html;
                    }
                }
            ]],
    ...
    function liveList(deviceSerial,qyname) {
        openDialog(qyname+"直播列表信息", ctx + '/zxjkyj/hkvision/device-qy/live/index/'+deviceSerial, "80%", "60%", "");
    }

    function bind(deviceSerial, qyname) {
        openDialog(qyname + "摄像头列表", ctx + '/zxjkyj/hkvision/device-qy/camera/index/' + deviceSerial, "100%", "100%",
            "");
