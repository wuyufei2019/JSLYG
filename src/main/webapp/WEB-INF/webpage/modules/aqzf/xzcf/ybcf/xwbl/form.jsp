...
<head>
    ...
    <script type="text/javascript" src="${ctx}/static/model/js/aqzf/xzcf/ybcf/xwbl/index.js?v=1.1"></script>
</head>
<body>
     ...
     <form id="inputForm" action="${ctx}/xzcf/ybcf/xwbl/${action}"  method="post" class="form-horizontal" >
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				...
				<tr>
					<td class="width-15 active"><label class="pull-right">询问人（多选）：</label></td>
				    <td class="width-35"><input id="M12" name="M12" type="text"  class="easyui-combobox" value="${xwbl.m12 }"  data-options="validType:'length[0,25]', editable:true , multiple:true ,valueField: 'text',textField: 'text',url:'${ctx}/aqzf/zfry/idlist' " style="height: 30px;width: 100%" /></td>
				...
				<tr>
				    ...
				    <td class="width-85" colspan="3"><textarea id="textarea_kindM17" name="M17" style="width:100%;height:500px;visibility:hidden;">${xwbl.m17 }</textarea></td>
				</tr>
				
				<input type="hidden" name="ID2" value="${xwbl.ID2}" />
				<input type="hidden" id="ID" name="ID" value="${xwbl.ID}" />
...
<script type="text/javascript">
	...
	$('#M16').textbox({  
    	onChange: function(value) {
	        var dczt = $("#M16").textbox("getValue");
	        var html = "我们是"+'${ssqmc}'+"应急管理局的行政执法人员"+'${xwbl.m12 }'+"，证件号码为"+'${zjh}'+"，这是我们的证件（出示证件）。我们依法就"+dczt+"的有关问题向您了解情况，您有如实回答问题的义务，也有陈述、申辩和申请回避的权利。您听清楚了吗？答：听清楚了。问：你是否申请回避？答：不申请。";
  			$("#ksxw").html(html);
	    }
	});
    	...
    	onChange: function(value) {
			...
			$.ajax({
  				...
  				success:function(data){
  					var html = "我们是"+'${ssqmc}'+"应急管理局的行政执法人员"+data.m15+"，证件号码为"+data.zjh+"，这是我们的证件（出示证件）。我们依法就"+'${xwbl.m16 }'+"的有关问题向您了解情况，您有如实回答问题的义务，也有陈述、申辩和申请回避的权利。您听清楚了吗？答：听清楚了。问：你是否申请回避？答：不申请。";
  					$("#ksxw").html(html);
  				}
  			});
	    }
	...
	function DiGui() {                
		$.ajax({  
			type: 'post',
		    url:'${ctx}/xzcf/ybcf/xwbl/dscreate',
	...
	$(function(){
		var html = "我们是"+'${ssqmc}'+"应急管理局的行政执法人员"+'${xwbl.m12 }'+"，证件号码为"+'${zjh}'+"，这是我们的证件（出示证件）。我们依法就"+'${xwbl.m16 }'+"的有关问题向您了解情况，您有如实回答问题的义务，也有陈述、申辩和申请回避的权利。您听清楚了吗？答：听清楚了。问：你是否申请回避？答：不申请。";
  		$("#ksxw").html(html);
			...
			$.ajax({
  				...
  				success:function(data){
  					$.each(data, function(i, item) {
  						var question="<p><span style=\"color:#E53333;\"><strong>问</strong></span>："+item.text+"</p><p><span style=\"color:#337FE5;\"><strong>答</strong></span>：</p>"
  						text+=question;
  					});
