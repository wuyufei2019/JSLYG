			...
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				...
				<tr>
                    ...
                    <td class="width-35">
                        <c:if test="${cfdxlx == '2'}">
                            <input name="cfryname" id="cfryname" readonly value="${xwtz.cfryname }" class="easyui-textbox" style="width: 100%;height: 30px;" data-options="validType:['length[0,25]']" />
                            <input type="hidden" name="ID2" value="${xwtz.ID2}">
                        </c:if>
                        <c:if test="${cfdxlx == null || cfdxlx == '1' || cfdxlx == ''}">
                            <input id="qyname" name="ID2" type="text" readonly class="easyui-combobox" value="${xwtz.ID2}"  style="height: 30px;width: 100%" data-options="required:'true',valueField: 'id',textField: 'text',url:'${ctx}/bis/qyjbxx/idjson',prompt: '若不存在该企业，请先添加!'" />
                        </c:if>
                    </td>
				...
				<tr>
				    ...
				    <td class="width-35"><input id="M6" name="M6" type="text"  class="easyui-combobox" value="${xwtz.m6 }"  data-options="panelHeight:'142px',editable:true ,valueField: 'text',textField: 'text',url:'${ctx}/aqzf/zfry/idlist',validType:'length[0,25]'" style="height: 30px;width: 100%" /></td>
...
<script type="text/javascript">
	$(function(){
	    ...
	    $("input",$("#qyname").next("span")).blur(function(){
			var he = /^[0-9]+.?[0-9]*$/;  
			var qyflag = $("#qyname").combobox("getValue");
			if(!he.test(qyflag) && qyflag != ''){
				$("#qyname").combobox("setValue","");
			}
		})
	});
