     ...
     <form id="inputForm" action="${ctx}/aqzf/jcjl/${action}"  method="post" class="form-horizontal" >
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				...
				<tr>
					<td class="width-20 active"><label class="pull-right">检查人员（多选）：</label></td>
					<td class="width-30" colspan="3"><input type="text" id="M8" name="M8" class="easyui-combobox" value="${jcjl.m8 }" style="width: 100%;height: 30px;" data-options="panelHeight:'142px', editable:false ,valueField: 'text', multiple:true, textField: 'text',url:'${ctx}/aqzf/zfry/idlist' "/></td>
				</tr>
				...
				<tr>
				    <td align="center" colspan="4">
						<a class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" onclick="addNr(${jcjl.m15})" title="其他问题"><i class="fa fa-plus"></i> 其他问题</a>
					</td>	
				</tr>
				...
				<c:if test="${not empty jcjl.id}">
					...
					<input type="hidden" name="M16" value="${jcjl.m16}" />
					<input type="hidden" name="M17" value="${jcjl.m17}" />
					<input type="hidden" name="M14" value="${jcjl.m14}" />
					<input type="hidden" name="M15" value="${jcjl.m15}" />
...
initData(version);
	...
	$('#inputForm').form({    
	    onSubmit: function(){    
	    	...
	    	if(isValid&&flag){
	    		flag=false;
	    		$("input[name='qtwtms']").each(function(index,item){
