...
window.onload = function() {
	...
	map.on('loadComplete', function() {
		...
		// 获取人员点标注  coords
		addMarkers(warningMarkerInfo.z,warningMarkerInfo.x,warningMarkerInfo.y,1);
	});
...
// 获取警报点信息
function getWarningCoord(rowId){
    $.ajax({
        type:'get',
        url:ctx+"/lydw/rydw/hisgjlist",
        data:rowId,
        success: function(data){
            showdata=jQuery.parseJSON(data);
            if(showdata.length>0){
            	gid = showdata[0].z;
            	var popmarkerInfo = showdata[0].name+"_"+showdata[i].bm+"_"+showdata[i].gw+"_"+showdata[i].uptime;
                warningMarkerInfo = {
                		x: showdata[0].x,
                		y: showdata[0].y,
                		z: showdata[0].z,
...
function addMarkers(warningMarkerInfo, markerImg) {
	removeMarkers();
	var group = map.getFMGroup(map.groupIDs[warningMarkerInfo.z - 1]);
