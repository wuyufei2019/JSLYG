...
<head>
    ...
    <script type="text/javascript" src="${ctx}/static/model/js/lydw/dzwl/index.js?v=1"></script>
...
<div class="easyui-layout" style="height: 100%">
    ...
    <div data-options="region:'west',split:true" style="width: 30%">
            <div id="lydw_dzwl_tb" style="padding:5px;height:auto;"  >
                ...
                <input type="text" name="lydw_dzwl_cx_name" class="easyui-textbox"  style="height: 30px;width: 120px;" data-options="prompt: '围栏名称'"/>
    ...
    <div id="cas" data-options="region:'center',split:true" style="width: 68%">
        ...
        <div class="operating">
            ...
            <button class="btn btn-default" onclick="cleanFence()" title="非立即删除围栏信息，只是在目前操作中清除">清除围栏</button>
            <button class="btn btn-default" onclick="createFence()">生成围栏</button>
            <button class="btn btn-default" onclick="saveMarker()" title="保存后将重置围栏信息至最新状态">保存围栏状态</button>
        </div>
    </div>
</div>
...
function saveMarker(){
    if(fenceState == 1) {
        ...
        layer.confirm('确认保存该电子围栏？', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type:'get',
                url:ctx+"/lydw/dzwl/adddzwl",
