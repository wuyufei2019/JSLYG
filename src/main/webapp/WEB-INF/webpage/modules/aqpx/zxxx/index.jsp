...
<c:if test="${empty kclist }">
    <div style="height:80px; color: #03A9F3;text-align:center; 60px;background-color:#F8F8F8;padding:26px 0 0px 60px; ">
        <h3 style="font-size: 24px;color: #5CB6F6">暂无学习课程</h3></div>
</c:if>
<c:if test="${!empty kclist }">
    <div style="height:80px; color: #03A9F3;padding-left:30px;margin-bottom: 30px;background-color:#F8F8F8;padding:26px 0 0px 60px; ">
        <h3 style="font-size: 24px;color: #5CB6F6">选择需要学习的课程</h3></div>
</c:if>
<div class="aqpx_zxxx_top">
    ...
    <c:forEach items="${kclist}" var="st" varStatus="status">
        <c:if test="${kclist[status.index-1].jh ne kclist[status.index].jh}">
            <div class="syiconbox">
                <h5 class="title">计划名称:${st.jh}&emsp;<small style="font-size: 14px">
                    开始时间：<fmt:formatDate pattern="yyyy-MM-dd" value="${st.m5}"/>&emsp;截止时间:<fmt:formatDate
                        pattern="yyyy-MM-dd" value="${st.m6}"/></small>
                </h5>
            </div>
        </c:if>
