...
.subject:hover .pic{
	background:url('${ctx}/static/model/images/aqpx/apply-blue.png') no-repeat center;
}
.pic{
	float:left;
	width:90px;
	...
	background:url('${ctx}/static/model/images/aqpx/apply-gray.png') no-repeat center;
...
<c:if test="${empty kclist }">
	<div style="height:80px; color: #03A9F3;text-align:center; 60px;background-color:#F8F8F8;padding:26px 0 0px 60px; "><h3 style="font-size: 24px;color: #5CB6F6">暂无相关课程信息</h3></div>
</c:if>
<c:if test="${!empty kclist }">
	<div style="height:80px; color: #03A9F3;padding-left:30px;margin-bottom: 60px;background-color:#F8F8F8;padding:26px 0 0px 60px; "><h3 style="font-size: 24px;color: #5CB6F6">三级安全教育培训学习课程</h3></div>
</c:if>
...
<c:forEach items="${kclist}" var="st" varStatus="status">
	<div class="subject" style="float: left;margin: 13px 13px;" >
		...
		<div class='content'>
			<span style="font-size: 15px;font-weight: bold;color: #242E34">${st.m1}</span>
			<br/>
			<span style="font-size: 14px;line-height: 26px;color: #929292">课程类别:三级教育</span>
			<br/>
			<span style="font-size: 14px;line-height: 26px;color: #929292">截止时间: 无限制</span>
		</div>
		<div style="clear: both;text-align:center">
		<hr style="border-top:1px solid #eeeeee; height:1px;margin:20px 0px 15px 0px;">
		<button class="exam" onclick="selectkc('${st.ID}')" title="开始学习">开始学习</button>
		</div>
	</div>
</c:forEach> 
</div>
