...
<head>
    ...
    <style type="text/css">
        ...
        #instantleakagedituContent label {
            max-width: none;
        }
...
<div id="instantleakage_result_win" style="display: none;padding-top: 5px;">
    <div style="text-align: center;">模拟时间：<input id="aca_ts" class="easyui-timespinner" style="width:80px;"
                                                 required="required"
                                                 data-options="min:'00:00:00',showSeconds:true,highlight:1,disabled:'true' "/>
        <a href='javascript:void(0)' class='btn btn-danger btn-xs' onclick="timeStop();"><i class='fa fa-pause'></i> 暂停</a>
        <a href='javascript:void(0)' class='btn btn-primary btn-xs' onclick="timeGo();"><i class='fa fa-play'></i>
            继续</a>
    </div>
</div>
...
<script type="text/javascript">
    ...
    if (usertype != '' && usertype != null) {
        var lng2 = '${lng2}';//经度
        var lat2 = '${lat2}';//纬度
        initMap("instantleakagedituContent", lng2, lat2, 16);
    } else {
        var lng = '${lng}';//经度
        var lat = '${lat}';//纬度
        //初始化地图
        initMap("instantleakagedituContent", lng, lat, 16);
    }
    ...
    $('#bis_cjxx_qy_name').combobox({
        onChange : function(n,o){
            ...
            $.ajax({
                url: ctx + '/sghgjs/instantleakage/getjwd',
                type: "get",
                data: {"qyid": qyid},
                dataType: "json",
                success: function (data) {
