...
public class XzcfCommonQzzxDao extends BaseDao<XZCF_YbcfQzzxEntity, Long> {
	...
	public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql = " SELECT TOP "+ mapData.get("pageSize")
				+ " * FROM ( SELECT ROW_NUMBER() OVER (ORDER BY b.id DESC)  AS RowNumber,a.id,a.qzzxsj,a.number,a.dsname,a.clname from xzcf_ybcfqzzx a "
	...
	/*	if (mapData.get("startdate") != null && mapData.get("startdate") != "") {
		...
		if (mapData.get("apptj") != null && mapData.get("apptj") != "") {
			content = content + " AND (a.dsname like '%" + mapData.get("apptj")+ "%' OR a.number like '%" + mapData.get("apptj")+ "%')";
		}
