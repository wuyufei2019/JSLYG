...
import com.cczu.aqzf.model.entity.AQZF_SafetyCheckUnitEntity;
...
public class AqzfJcjlDao extends BaseDao<AQZF_SafetyCheckRecordEntity, Long> {
    ...
    public List<Map<String, Object>> dataGridNr2(String id) {
        String sql = " SELECT a.m1 jcjg,a.m2 jcwt,a.m3 wtms,a.m4 pic,a.m5 zgwt, c.m1 jcdy,b.m2 jcnr,b.m4 ,b.m5,b.m6,b.id "
     ...
     * @return
     */
    public AQZF_SafetyCheckRecordEntity findJcjlByfaid(long faid) {
        String sql = " SELECT * from aqzf_safetycheckrecord where S3=0 and id1 = " + faid;
        List<AQZF_SafetyCheckRecordEntity> list = findBySql(sql, null, AQZF_SafetyCheckRecordEntity.class);
        if (list.size() < 1) {
            return null;
        }
     ...
     * @return
     */
    public XZCF_YbcfLaspEntity findLaspByjlid(String jlid) {
        String sql = "SELECT a.* FROM xzcf_ybcflasp a LEFT JOIN aqzf_safetycheckrecord b ON a.id2 = b.id WHERE b.id = " + jlid;
     ...
     * @return
     */
    public XZCF_YbcfLaspEntity findLaspByfaid(String faid) {
        String sql = "SELECT a.* FROM xzcf_ybcflasp a LEFT JOIN aqzf_safetycheckrecord b ON a.id2 = b.id LEFT JOIN aqzf_safetycheckscheme c ON b.id1 = c.id WHERE c.id = " + faid;
        List<XZCF_YbcfLaspEntity> list = findBySql(sql, null, XZCF_YbcfLaspEntity.class);
        if (list.size() < 1) {
            return null;
        }
