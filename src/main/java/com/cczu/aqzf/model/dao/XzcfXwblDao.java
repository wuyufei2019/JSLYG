...
public class XzcfXwblDao extends BaseDao<XZCF_InterrogationRecordEntity, Long> {
	...
	public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql = "SELECT TOP " + mapData.get("pageSize") + "a.* FROM ("+
		           "SELECT ROW_NUMBER() OVER(order by b.id desc,a.id) rownum,a.*,b.number labh from xzcf_interrogationrecord a left join xzcf_ybcflasp b on b.id = a.id2 left join bis_enterprise c on c.id = b.id1 left join xzcf_enquirynote d on d.id3 = b.id left join t_user u on b.userid = u.id WHERE a.s3 = 0 and b.s3=0 and c.s3=0 and d.s3=0 "+content +") a "+
	...
	public String content(Map<String, Object> mapData) {
		...
		if(mapData.get("bh")!=null&&mapData.get("bh")!=""){
			content = content + " AND b.number like '%"+mapData.get("bh")+"%' "; 
		}
		if(mapData.get("xzqy")!=null&&mapData.get("xzqy")!=""){
			content = content +" AND u.xzqy like '"+mapData.get("xzqy")+"%' ";
		}
		if(mapData.get("apptj")!=null&&mapData.get("apptj")!=""){
			content = content + " AND (b.number like '%"+mapData.get("apptj")+"%' OR a.m8 like '%"+mapData.get("apptj")+"%')"; 
		}
		return content;
	}
	...
	public List<XZCF_InterrogationRecordEntity> findByLaid(long laid) {
		String sql = "SELECT * from xzcf_interrogationrecord WHERE s3 = 0 and id2 = "+laid +" order by id";
		List<XZCF_InterrogationRecordEntity> list=findBySql(sql,null, XZCF_InterrogationRecordEntity.class);
		return list;
	}
}
