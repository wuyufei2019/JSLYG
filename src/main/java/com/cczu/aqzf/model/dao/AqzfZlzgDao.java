...
public class AqzfZlzgDao extends BaseDao<AQZF_ReformEntity, Long> {
    ...
    public void deleteInfo(long id) {
        String sql = " UPDATE aqzf_reform SET S3=1 WHERE id=" + id;
        updateBySql(sql);
    }
    ...
    public Map<String, Object> findInforById(Long id) {
        String sql = "SELECT a.*,b.m1 qyname,b.id qyid FROM aqzf_reform a left join bis_enterprise b on b.id=a.id2  " +
    ...
    public List<Map<String, Object>> dataGridForH5(Map<String, Object> mapData) {
        String content = content(mapData);
        String sql = "SELECT top 5 * FROM (SELECT a.id,a.m3,b.m1 qyname,a.m9,DATEDIFF(DAY, GETDATE(), a.m3) fcts "
                + "FROM aqzf_reform a left join bis_enterprise b on b.id=a.id2 left join aqzf_safetycheckrecord c on c.id=a.id1 "
                + "left join aqzf_safetycheckscheme d on d.id=c.id1 LEFT JOIN aqzf_safetycheckplan e on e.id=d.id1 "
                + "LEFT JOIN t_user f on f.id=e.id1 WHERE a.S3=0 AND b.S3=0 AND c.S3=0 AND d.S3=0 AND e.S3=0 AND a.m9 = 0 " + content + ") zz "
                + "WHERE fcts >= 0 ORDER BY fcts";
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
        return list;
    }
}
