...
public class XzcfCssbblDao extends BaseDao<XZCF_CssbblEntity, Long> {
	...
	public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql = "SELECT TOP " + mapData.get("pageSize") + "a.* FROM ("+
		           "SELECT ROW_NUMBER() OVER(order by b.id desc) rownum,a.*,b.number labh FROM xzcf_cssbbl a LEFT JOIN xzcf_ybcflasp b ON a.id1 = b.id left join t_user u on b.userid = u.id "
	...
	public String content(Map<String, Object> mapData) {
		...
		if(mapData.get("labh")!=null&&mapData.get("labh")!=""){
			content = content + "AND b.number like '%"+mapData.get("labh")+"%' "; 
		}
		if(mapData.get("xzqy")!=null&&mapData.get("xzqy")!=""){
			content = content +" AND u.xzqy like '"+mapData.get("xzqy")+"%' ";
		}
		if(mapData.get("apptj")!=null&&mapData.get("apptj")!=""){
			content = content + "AND (b.number like '%"+mapData.get("apptj")+"%' OR a.m11 like '%"+mapData.get("apptj")+"%')"; 
		}
		return content;
	}
