...
import com.cczu.model.zyaqgl.service.AqglDhzyFxService;
...
public class PageAqglDhzyController extends BaseController {
	 ...
	 */
	@RequiresPermissions("zyaqgl:dhzy:add")
	@RequestMapping(value = "createAqcs/{id1}/{ids}", method = RequestMethod.GET)
	@ResponseBody
	public String createAqcs(@PathVariable("id1") Long id1,@PathVariable("ids") String ids) {
		String datasuccess="编制成功";
		//批量关联安全措施
		String[] arrids = ids.split(",");
		for(int i=0;i<arrids.length;i++){
			AQGL_Zyaq_Aqcs aqcs=new AQGL_Zyaq_Aqcs();
			AQGL_SaftyMeasure aqcs2=aqglaqcsService.findInforById(Long.parseLong(arrids[i]));
			aqcs.setID1(id1);
