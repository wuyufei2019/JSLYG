...
//				"M33_3", "M34", "M35", "M36", "M36_1", "M37", "M38", "M39", "M40", "M41", "M42", "M43", "M44", "M45"
    ...
    public void ExportQyxxTotalAll(HttpServletResponse response, Map<String, Object> map) {
        ...
        String[] title = {"企业名称", "所属乡镇", "车间", "仓库", "物料", "储罐", "特种设备", "安全培训", "生产设备", "公用工程", "员工",
                "危险工艺", "重大危险源", "安全设施", "粉尘", "受限空间", "能源", "承包商", "冶金", "作业班次", "风险点", "专家库"};
        String[] keys = {"m1", "m11_3", "cj", "ck", "wl", "cg", "tzsb", "aqpx", "scsb", "gygc", "yg",
                "wxgy", "wxy", "aqss", "fc", "sxkj", "ny", "cbs", "yj", "zybc", "fxd", "zjk"};
        new ExportExcel(fileName, title, keys, list, response);
    }
    ...
    @Override
    public boolean checkQyname(String qyName, Long qyid) {
        boolean flag = true;
        List<BIS_EnterpriseEntity> list = bisQyjbxxDao.find(Restrictions.eq("S3", 0),
                Restrictions.eq("M1", qyName));
        if (list != null && list.size() > 0) {
            BIS_EnterpriseEntity bis = list.get(0);
            if (qyid != null && qyid.equals(bis.getID()))
                flag = true;
            else
                flag = false;
        } else {
    ...
    public List<Map<String, Object>> getMarker(String qyname) {
        return bisQyjbxxDao.getMarker(qyname);
    }
