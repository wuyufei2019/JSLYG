...
public class TargetAqzrsscDao extends BaseDao<Target_SafetyDutyAgreementRec, Long>{
	...
	public List<Map<String,Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql =" SELECT top "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS"
				+ " RowNumber,a.id aid,a.title,a.year,a.url zrsurl,b.* ,u.id uid ,u.m1 name FROM"
				+ " target_safetydutyagreement a left join bis_enterprise bis on a.id1=bis.id"
				+ " left join bis_employee u on charindex(','+cast(u.id4 as VARCHAR)+',',','+a.departments+',')!=0"
				+ " left join target_safetydutyagreementrec b on a.id = b.id1 and b.id3=u.id "
				+ " WHERE a.S3=0 and bis.s3=0 and (b.s3=0 or b.id is null) and u.s3 = 0 "
    ...
    public int getTotalCount(Map<String, Object> mapData) {
		...
		String sql="SELECT COUNT(a.id) sum FROM"
				+ " target_safetydutyagreement a left join bis_enterprise bis on a.id1=bis.id"
				+ " left join bis_employee u on charindex(','+cast(u.id4 as VARCHAR)+',',','+a.departments+',')!=0"
				+ " left join target_safetydutyagreementrec b on a.id = b.id1"
				+ " WHERE a.S3=0 and bis.s3=0 and (b.s3=0 or b.id is null) and u.s3 = 0 "+content;
    ...
    public void deleteInfo(Long id) {
		String sql=" UPDATE target_safetydutyagreementrec SET S3=1 WHERE ID="+id;
		updateBySql(sql);
	}
