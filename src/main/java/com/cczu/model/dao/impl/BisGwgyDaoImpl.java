...
public class BisGwgyDaoImpl extends BaseDao<BIS_DangerProcessEntity,Long> implements IBisGwgyDao {
	...
	public Map<String,Object> findById(Long id) {
		...
		String sql1 ="SELECT c.label as label FROM bis_dangerprocess b left join t_dict c on c.VALUE=b.m1 and c.type='gwgy'"
				+ " WHERE b.s3=0 AND b.ID="+id;
		List<BIS_DangerProcessEntity> list=findBySql(sql, null,BIS_DangerProcessEntity.class);
		List<Map<String,Object>> label=findBySql(sql1, null,Map.class);
		if(list.size()>0){
			map.put("danger", list.get(0));
			map.put("label", label.get(0));
			return map;
		}
		return null;
	}
	...
	public List<Map<String, Object>> gwgy(Map<String, Object> mapData) {
		// TODO Auto-generated method stub
		String content=content(mapData);
		String sql =" SELECT top "+mapData.get("pageSize")+" ID,S1,S2,S3,ID1,M1,M2,M3,M4,LABEL FROM ( SELECT ROW_NUMBER() OVER (ORDER BY b.id desc) AS RowNumber,b.*,t.LABEL FROM bis_dangerprocess b,t_dict t "
				+ "WHERE b.M1=t.VALUE and b.s3=0 "+content+" ) "
