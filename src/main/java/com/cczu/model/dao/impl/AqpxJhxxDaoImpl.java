...
public class AqpxJhxxDaoImpl extends BaseDao<AQPX_PlanEntity, Long> implements IAqpxJhxxDao {
	...
	public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
		...
		if(User.USERTYPE.QY.getUsertype().equals(UserUtil.getCurrentShiroUser().getUsertype())){
			 sql =" SELECT top "+mapData.get("pageSize")+" * FROM( SELECT ROW_NUMBER() OVER ( ORDER BY p.id) AS RowNumber, p.*,ISNULL(a.kscount,0) cjks,ISNULL((b.ykscount-ISNULL(a.kscount,0)),0) wcjks,ISNULL(c.hg,0) hg, ISNULL(d.bhg,0) bhg"
					...
					+ " LEFT JOIN (select count(a.id) ykscount,b.id planid from t_user a left join aqpx_plan b on ','+b.id3+',' like '%,'+CAST(a.departmen as varchar(200))+',%' where b.s3=0 and (a.DEL_FLAG=0 or a.DEL_FLAG IS NULL) GROUP BY b.id)b on p.id=b.planid"
		...
		}else if(User.USERTYPE.AJ.getUsertype().equals(UserUtil.getCurrentShiroUser().getUsertype())){

			 sql =" SELECT top "+mapData.get("pageSize")+" * FROM( SELECT ROW_NUMBER() OVER ( ORDER BY p.id) AS RowNumber, p.*,ISNULL(a.kscount,0) cjks,ISNULL((b.ykscount-ISNULL(a.kscount,0)),0) wcjks,ISNULL(c.hg,0) hg, ISNULL(d.bhg,0) bhgis,bis.m1 qyname"
					+ " FROM aqpx_plan p LEFT JOIN (select count(DISTINCT(a.id2)) kscount,a.id4 planid from aqpx_examresults a left join aqpx_plan b on a.id4=b.id where a.id4 is not NULL GROUP BY a.id4)a on p.id=a.planid"
					+ " LEFT JOIN (select count(a.id) ykscount,b.id planid from t_user a left join aqpx_plan b on ','+b.id3+',' like '%,'+CAST(a.departmen as varchar(200))+',%' where b.s3=0 and (a.DEL_FLAG=0 or a.DEL_FLAG IS NULL) GROUP BY b.id)b on p.id=b.planid"
					+ " LEFT JOIN ( SELECT COUNT ( DISTINCT ( x.id2 )) hg, x.id4 planid FROM (SELECT *  FROM ( SELECT *, ROW_NUMBER () OVER ( PARTITION BY id2,id4 ORDER BY CONVERT(Numeric,m1) DESC ) AS rn FROM aqpx_examresults ) AS t WHERE t.rn= 1) x WHERE x.m3 = '合格' GROUP BY x.id4 )c on p.id=c.planid"
					+ " LEFT JOIN ( SELECT COUNT ( DISTINCT ( y.id2 )) bhg, y.id4 planid FROM (SELECT *  FROM ( SELECT *, ROW_NUMBER () OVER ( PARTITION BY id2,id4 ORDER BY CONVERT(Numeric,m1) DESC ) AS rn FROM aqpx_examresults ) AS t WHERE t.rn= 1) y WHERE y.m3 = '不合格' GROUP BY y.id4 )d on p.id=d.planid "
					+ " LEFT JOIN bis_enterprise bis on p.id1 = bis.id WHERE p.S3=0 "+content+ com.cczu.util.common.StringUtils.qyQueryJGLX(mapData,"bis")+") as s WHERE RowNumber > "+mapData.get("pageSize")+"*("+mapData.get("pageNo")+"-1) " ;
		}
