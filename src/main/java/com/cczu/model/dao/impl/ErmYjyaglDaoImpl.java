...
public class ErmYjyaglDaoImpl extends BaseDao<AQJG_SafetyRecord, Long> implements IErmYjyaglDao {
	...
	@Override
	public void addInfo(AQJG_SafetyRecord erm) {
		save(erm);
	}
	...
	public List<Map<String, Object>> getqylistapp(Map<String, Object> tmap) {
		...
		String sql="SELECT TOP "+tmap.get("pageSize")+" * FROM ("+
				"SELECT ROW_NUMBER() OVER (ORDER BY id) AS RowNumber,id,m1,m11,m11_1,m11_2,m11_3,m5,m6,(case M36 when '1' then '工贸' when '2' then '化工' end) as m36,(case when (M39= '1') then '是' else '否'end) m39,m19,m44 from bis_enterprise b where b.s3=0 and id in(SELECT DISTINCT (id1) FROM aqjg_saftyrecord where s3=0 and id1!=0 AND M3 ='yjya') "+content 
