...
public class BisQyjbxxDaoImpl extends BaseDao<BIS_EnterpriseEntity, Long> implements IBisQyjbxxDao {
	...
	public List<Map<String,Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql="SELECT TOP "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS RowNumber,bis.id,bis.m1,bis.m19,bis.m21,"
				+ "bis.m23,bis.m25,bis.m39,bis.m40,bis.m47,bis.m48,bis.m44,bis.m33_3,bis.ktdate,bis.dqdate,bis.state,tv.id1 hastv,fa.id1 as hasfa,bt.id1 as hasbt"
...
//		updateBySql(sql);
	...
	public String content(Map<String, Object> mapData) {
		...
		if(mapData.get("bis_ids")!=null&&mapData.get("bis_ids")!=""){
			content = content + "AND bis.id not in ("+mapData.get("bis_ids")+") ";
		}
		...
		if(mapData.get("state")!=null&&mapData.get("state")!=""){
			if ("0".equals(mapData.get("state").toString())) {// 停用
				content = content + "AND bis.state='"+mapData.get("state")+"' "; 
			} else if ("1".equals(mapData.get("state").toString())) {// 启用
				content = content + "AND bis.state is null "; 
			}
		}
		//添加创建者查询条件
		...
		if(mapData.get("jglx")!=null&&mapData.get("jglx")!=""){
			if (StringUtils.toInteger(mapData.get("jglx")) == 10){
				content = content + "AND bis.m36= '1' OR bis.m36 = '2'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 11){
				content = content + "AND bis.m36= '1' OR bis.m36 = '3'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 12){
				content = content + "AND bis.m36= '1' OR bis.m36 = '9'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 13){
				content = content + "AND bis.m36= '2' OR bis.m36 = '3'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 14){
				content = content + "AND bis.m36= '2' OR bis.m36 = '9'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 15){
				content = content + "AND bis.m36= '3' OR bis.m36 = '9'";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 16){
				content = content + "AND bis.m36= '1' OR bis.m36 = '2' OR bis.m36 = '3' ";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 17){
				content = content + "AND bis.m36= '1' OR bis.m36 = '3' OR bis.m36 = '9' ";
			}else if (StringUtils.toInteger(mapData.get("jglx")) == 18){
				content = content + "AND bis.m36= '9' OR bis.m36 = '2' OR bis.m36 = '3' ";
			}else {
	...
	public List<Map<String, Object>> findZdzdMapList(Map<String, Object> mapdata) {
		String content=content(mapdata);
		String sql="SELECT distinct bis.id,bis.m16 as pointx,bis.m17 as pointy,bis.m1 as title,bis.m33 as address," +
				"'/static/model/images/map/bdmap_icon_c01.png' icon,'zdjg' type "
				+ "FROM bis_mat a left join bis_enterprise bis on bis.id=a.id1 WHERE a.s3=0 and bis.s3=0 and a.m12=1"
				+ content+"union all "
				+ "SELECT distinct bis.id,bis.m16 as pointx,bis.m17 as pointy,bis.m1 as title,bis.m33 as address," +
				"'/static/model/images/map/bdmap_icon_y01" +
				".png' icon,'wxgy' type FROM "
				+ "bis_dangerprocess b left join bis_enterprise bis on bis.id=b.id1 where bis.S3=0 and b.S3=0 "+content
				+" union all "
				+ "SELECT distinct bis.id,bis.m16 as pointx,bis.m17 as pointy,bis.m1 as title,bis.m33 as address," +
				"'/static/model/images/map/bdmap_icon_h01" +
				".png' icon,'wxy' type "
	...
	public List<Map<String, Object>> ExportQyxxTotalAll(Map<String, Object> map) {
		String content=content(map);
		String sql =" SELECT  bis.id,bis.m1,bis.m11_3,tuser.name,isnull(a.num,'无') cj,isnull(b.num,'无') wl,isnull(z.num,'无') zjk,isnull(c.num,'无') ck,isnull(d.num,'无') cg,isnull(e.num,'无') tzsb,"+
					...
					" LEFT JOIN (select qyid,CAST(COUNT(*) as varchar(10)) num from erm_emergencyresexpert WHERE s3=0 group by qyid) z on bis.id=z.qyid"+
	...
	public Map<String, Object> getTotalViewById(Long qyid) {
		String sql =" SELECT bis.id,isnull(w.num,0) fxd,isnull(a.num,0) cj,isnull(b.num,0) wl,isnull(c.num,0) ck,isnull(d.num,0) cg,isnull(x.num,0) pjbg,"+
				...
				" isnull(q.num,0) fc,isnull(r.num,0) sxkj,isnull(s.num,0) ny,isnull(t.num,0) cbs,isnull(u.num,0) yj,isnull(v.num,0) zybc,isnull(y.num,0)zjk, isnull(z.num,0)qybg from bis_enterprise bis "+
				" LEFT JOIN (select ID1,COUNT(*) num from bis_workshop WHERE s3=0 group by ID1 ) a on bis.id=a.ID1 "+
				" LEFT JOIN (select ID1,COUNT(*) num from bis_mat WHERE s3=0 group by ID1) b on bis.id=b.ID1"+
				" LEFT JOIN (select ID1,COUNT(*) num from bis_storage WHERE s3=0 group by ID1) c on bis.id=c.ID1"+
				" LEFT JOIN (select ID1,COUNT(*) num from bis_tank WHERE s3=0 group by ID1) d on bis.id=d.ID1"+
				...
				" LEFT JOIN (select ID1,COUNT(*) num from bis_workorder WHERE s3=0 group by ID1) v on bis.id=v.ID1"+
				" LEFT JOIN (select ID1,COUNT(*) num from fxgk_accidentrisk WHERE s3=0 group by ID1) w on bis.id=w.ID1"+
				" LEFT JOIN (select qyid,COUNT(*) num from erm_emergencyresexpert WHERE s3=0 group by qyid) y on bis.id=y.qyid"+
				" LEFT JOIN (select qyid,COUNT(*) num from bis_infochange WHERE s3=0 group by qyid) z on bis.id=z.qyid"+
	...
	根据企业名称获取marker点
	 */
	public List<Map<String,Object>> getMarker(String qyname){
		String sql = "SELECT * FROM bis_enterprise a WHERE a.s3 = 0 AND a.m1 LIKE '"+qyname+"%'";
		List<BIS_EnterpriseEntity> bislist=findBySql(sql, null,BIS_EnterpriseEntity.class);
		List<Map<String,Object>> maplist=new ArrayList<Map<String, Object>>();
		for (BIS_EnterpriseEntity bis : bislist) {
			if(bis.getM1()!=null){
