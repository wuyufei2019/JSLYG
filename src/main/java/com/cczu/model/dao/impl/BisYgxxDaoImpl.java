...
public class BisYgxxDaoImpl extends BaseDao<BIS_EmployeeEntity, Long> implements IBisYgxxDao {
	 ...
	 * @return
	 */
	public List<BIS_EmployeeEntity> findSameList(Map<String, Object> map) {
		String sql=" select * from bis_employee where s3=0 and ID3 = "+map.get("qyid")+" and m2 = '"+map.get("m2")+"'";
		List<BIS_EmployeeEntity> list=findBySql(sql, null,BIS_EmployeeEntity.class);
		return list;
	}
	...
	public int getTotalCountByQyid(Map<String, Object> map) {
		String content = content(map);
		String sql="SELECT COUNT(*) SUM FROM bis_employee a LEFT JOIN t_department c ON a.ID4 = c.ID  where a.S3=0 "+ content;
		List<Object> list=findBySql(sql);
		return (int) list.get(0);
	}
