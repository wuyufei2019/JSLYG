...
public class YhpcXjjlDao extends BaseDao<YHPC_CheckResultEntity, Long> {
    ...
    public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
        ...
        String sql = " SELECT top " + mapData.get("pageSize") + " * FROM (SELECT ROW_NUMBER() OVER (order by a.createtime DESC) AS RowNumber," +
                " a.id,d.m1 jcdname,d.type,d.id jcdid,b.name,a.createtime,c.NAME uname,a.checkresult,a.note,a.checkphoto,dep.m1 depname " +
                "FROM yhpc_checkresult a LEFT JOIN yhpc_checkplan b ON a.checkplan_id = b.id " +
                "LEFT JOIN t_user c ON c.ID = a.userid LEFT JOIN "
                + " fxgk_accidentrisk d ON d.id = a.checkpoint_id "
                + " LEFT JOIN t_department dep ON dep.id = c.departmen"
                + " WHERE 0 = 0" + content + " ) "
     ...
     *
     * @param mapData
     * @return 如果要查看最新记录把最后where 1=1 改为 h.rn<=1
     */
    ...
    public Map<String, Object> findInforById(Long id) {
        String sql = " SELECT a.id,d.m1 jcdname,d.type,d.id jcdid,b.name,a.createtime,c.NAME uname," +
                "(case a.checkresult when '0' then '无隐患' when '1' then '有隐患' end) checkresult,a.note,a.checkphoto" +
                ",a.checkpoint_id,a.checkpointtype "
                + "FROM yhpc_checkresult a LEFT JOIN yhpc_checkplan b ON a.checkplan_id = b.id LEFT JOIN t_user c ON " +
                "c.ID = a.userid LEFT JOIN fxgk_accidentrisk d ON d.id = a.checkpoint_id WHERE a.id=" + id;
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
        return list.get(0);
    }
    ...
    public int getajTotalCount(Map<String, Object> mapData) {
        ...
        String sql = "SELECT COUNT(*) FROM (select * from (SELECT ROW_NUMBER() OVER (partition by e.m1,d.jcdname ORDER BY a.createtime DESC) AS rn, a.id,e.m1 qyname,e.fid,d.jcdname,b.name,a.createtime,c.NAME uname,a.checkresult,a.note,a.checkphoto,a.checkpoint_id,a.checkpointtype,e.id2 xzqy,e.m36 "
                + "FROM yhpc_checkresult a LEFT JOIN yhpc_checkplan b ON a.checkplan_id = b.id LEFT JOIN t_user c ON c.ID = a.userid LEFT JOIN (SELECT a.id,a.checkpoint_id,b.m1 jcdname,b.id1 qyid FROM yhpc_checkresult a "
                + "LEFT JOIN fxgk_accidentrisk b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '1' AND b.s3 = 0 UNION SELECT a.id,a.checkpoint_id,b.name jcdname,b.id1 qyid FROM yhpc_checkresult a "
                + "LEFT JOIN yhpc_checkpoint b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '2' and b.usetype = '2') d ON d.id = a.id LEFT JOIN bis_enterprise e ON e.id = d.qyid "
    ...
    public List<Map<String, Object>> getExport(Map<String, Object> mapData) {
        ...
        String sql = " SELECT h.*,(CASE h.checkresult when 0 then '无隐患' when 1 then '有隐患' ELSE '' END) lx FROM (SELECT ROW_NUMBER() OVER (ORDER BY a.id) AS RowNumber,a.id,e.m1 qyname,d.jcdname,(case when b.name is null then '该班次已被删除' else b.name end) name,CONVERT(varchar(100), a.createtime, 120)createtime,c.NAME uname,a.checkresult,a.note,a.checkphoto "
                ...
                + "(SELECT a.id,b.m1 jcdname,b.id1 qyid FROM yhpc_checkresult a LEFT JOIN fxgk_accidentrisk b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '1' AND b.s3 = 0 "
                + "UNION SELECT a.id,b.name jcdname,b.id1 qyid FROM yhpc_checkresult a LEFT JOIN yhpc_checkpoint b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '2' and b.usetype = '2') d ON d.id = a.id "
    ...
    public List<Map<String, Object>> getExport2(Map<String, Object> mapData) {
        ...
        String sql = " SELECT z.*,(CASE z.checkresult when 0 then '无隐患' when 1 then '有隐患' ELSE '' END) lx FROM (SELECT h.* FROM "
                ...
                + "FROM yhpc_checkresult a LEFT JOIN yhpc_checkplan b ON a.checkplan_id = b.id LEFT JOIN t_user c ON c.ID = a.userid LEFT JOIN (SELECT a.id,b.m1 jcdname,b.id1 qyid FROM yhpc_checkresult a "
                + "LEFT JOIN fxgk_accidentrisk b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '1' AND b.s3 = 0 UNION SELECT a.id,b.name jcdname,b.id1 qyid FROM yhpc_checkresult a "
                + "LEFT JOIN yhpc_checkpoint b ON b.id = a.checkpoint_id WHERE a.checkpointtype = '2' and b.usetype = '2') d ON d.id = a.id LEFT JOIN bis_enterprise e ON e.id = d.qyid "
