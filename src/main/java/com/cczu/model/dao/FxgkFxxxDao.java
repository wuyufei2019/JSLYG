...
public class FxgkFxxxDao extends BaseDao<FXGK_AccidentRisk, Long> {
    ...
    public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
        String con1 = "ORDER BY depname,id1,m9";
        if (mapData.get("orderBy").equals("m9")) {
            con1 = "ORDER BY m9 " + mapData.get("order") + ",id1";
        }
        String content = content(mapData);
        String sql = " SELECT top  " + mapData.get("pageSize") + " * FROM (SELECT ROW_NUMBER() OVER (" + con1 + ") AS RowNumber, aaa.* FROM ( SELECT distinct a.*,dep.m1 depname, b.M1 qyname,z.id stoppointid "
    ...
    public List<Map<String, Object>> findPmtByQyid(Long id) {
        String sql = "SELECT a.m19 locx, a.m20 locy,a.m9 color, b.m33_3 pmt"
                + " FROM fxgk_accidentrisk a"
                + " LEFT JOIN bis_enterprise b ON b.id = a.id1"
                + " WHERE a.type=1 and a.S3 = 0 AND b.S3 = 0 AND a.id1 = " + id;
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
        return list;
    }
    ...
    public List<Map<String, Object>> getExport(Map<String, Object> mapData) {
        ...
        String content = content(mapData);
        String sql = " select a.*,dep.m1 depname,CONVERT(varchar(100), a.s1, 23) rq,(case a.m9 when '1' then '重大风险' when '2' then '较大风险' when '一般风险' then '黄' when '4' then '低风险' end)fxfj "
                + " from fxgk_accidentrisk a left join bis_enterprise b on a.id1=b.id "
                + " left join t_department dep on a.depid = dep.id "
                + " where a.type=1 and a.s3=0 and b.s3=0 " + content + "ORDER BY b.m1";
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
        return list;
    }
    ...
    public List<Map<String, Object>> getFXByqyid(Long id) {
        String sql = "SELECT a.m17,a.m1,a.areaname,a.m8,(case a.m9 when 1 then '红色' when 2 then '橙色' when 3 then '黄色'" +
                " when 4 then '蓝色' else '蓝色' end) m9,(case a.m9 when 1 then 'C00000' when 2 then 'FFA042' when 3 then 'FFD700' when 4 then '2894FF' else '2894FF' end) color, b.M1 qyname,b.id qyid FROM fxgk_accidentrisk a left join bis_enterprise b  on b.id=a.id1 WHERE a.type=1 and a.S3=0  AND b.S3=0 and b.id =" + id + "ORDER BY a.m9";
    ...
    public List<Map<String, Object>> getSgtypeByqyid(String qyid) {
        String sql = "SELECT a.m8 FROM fxgk_accidentrisk a left join bis_enterprise b  on b.id=a.id1 WHERE a.type=1 and a.S3=0  " +
                "AND b.S3=0 and b.id =" + qyid;
    ...
    public List<Map<String, Object>> getAllRLQylistDataGrid(Map<String, Object> mapData) {
        ...
        if (mapData.get("type") != null && mapData.get("type") != "") {
            sqladd += " and a.type='" + mapData.get("type") + "'";
        }
        ...
        String sql = " SELECT TOP "
                ...
                + "where a.type=1 and a.s3=0 " + content + ") a where 1=1 " + sqladd + ") as a WHERE a.RowNumber > "
...
//		}
    ...
    public List<Map<String, Object>> xjnrdataGrid3(Map<String, Object> mapData) {
        String sql = "SELECT top  " + mapData.get("pageSize") + " * FROM ("
                + "SELECT ROW_NUMBER() OVER (ORDER BY b.id) AS RowNumber,a.id,b.checktitle,b.dangerlevel,b.content,b.checkyes,b.checkno,c.error,c.ychid,c.yhzp,c.yhbz FROM yhpc_riskpoint_content a "
                + "LEFT JOIN yhpc_checkcontent b ON a.id2 = b.id LEFT JOIN (SELECT ych.dangerdesc yhbz,ych.checkcontent_id,1 error,ych.id ychid,ych.dangerphoto yhzp FROM yhpc_checkresult ycr LEFT JOIN yhpc_checkhiddeninfo ych ON ycr.id = ych.checkresult_id "
    ...
    public List<Map<String, Object>> xjnrdataGrid4(Map<String, Object> mapData) {
        String sql = "SELECT top  " + mapData.get("pageSize") + " * FROM ("
                + "SELECT ROW_NUMBER() OVER (ORDER BY b.id) AS RowNumber,a.id,b.checktitle,b.dangerlevel,b.content,b.checkyes,b.checkno,c.error,c.ychid,c.yhzp,c.yhbz FROM yhpc_checkpoint_content a "
                + "LEFT JOIN yhpc_checkcontent b ON a.id2 = b.id LEFT JOIN (SELECT ych.dangerdesc yhbz,ych.checkcontent_id,1 error,ych.id ychid,ych.dangerphoto yhzp FROM yhpc_checkresult ycr LEFT JOIN yhpc_checkhiddeninfo ych ON ycr.id = ych.checkresult_id "
                + "WHERE ycr.id = " + mapData.get("xjjlid") + ")c ON a.id2 = c.checkcontent_id where a.id1 = " + mapData.get("jcdid") + ") "
                + " as s WHERE  RowNumber > " + mapData.get("pageSize") + "*(" + mapData.get("pageNo") + "-1)";
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
    ...
    public int getxjnrTotalCount2(Map<String, Object> mapData) {
        String sql = "SELECT COUNT(*) FROM yhpc_riskpoint_content a "
    ...
    public void deleteXjnrByAccid(long id1) {
        String sql = " delete yhpc_riskpoint_content WHERE id1=" + id1;
        updateBySql(sql);
    }
    ...
    public List<Map<String, Object>> getAllByQyid(Map<String, Object> mapData) {
        // TODO Auto-generated method stub
        String content = content(mapData);
        String sql = " select a.id,a.m1,a.m19,a.m20,a.m9,(case a.m9 when '1' then '红' when '2' then '橙' when '3' then" +
                " '黄' when '4' then '蓝' end)fxfj,b.m1 qyname from fxgk_accidentrisk a left join bis_enterprise b on a" +
                ".id1=b.id where a.type=1 and a.s3=0 and b.s3=0  " + content + "ORDER BY b.m1";
        List<Map<String, Object>> list = findBySql(sql, null, Map.class);
        return list;
    }
}
