...
public class WghglWgxjjlDao extends BaseDao<YHPC_CheckResultEntity, Long> {
	...
	public List<Object> getXjjlUser(String xzqy) {
		String sql="SELECT DISTINCT userid from yhpc_checkresult a LEFT JOIN t_user u on u.id = a.userid where u.usertype = 0 AND xzqy like'"+xzqy+"%'";
		List<Object> list=findBySql(sql);
		return list;
	}
	...
	public List<Map<String, Object>> getExport(Map<String, Object> mapData) {
		String content=content(mapData);
		String sql ="SELECT (case a.checkresult when '0' then '无隐患' else '有隐患' end) checkresult,a.checkphoto,e.m1 wgname,b.name wgdname,d.m1 qyname,f.NAME uname,c.name bcname FROM yhpc_checkresult a "
				+ "LEFT JOIN yhpc_checkpoint b ON a.checkpoint_id = b.id LEFT JOIN yhpc_checkplan c ON a.checkplan_id = c.id "
				+ "LEFT JOIN bis_enterprise d ON b.id1 = d.id LEFT JOIN t_barrio e ON e.code = d.id2 "
				+ "LEFT JOIN t_user f ON f.ID = a.userid WHERE a.checkpointtype = '2' AND b.usetype = '1' AND d.s3 = 0 "+content ;
		List<Map<String, Object>> list=findBySql(sql,null,Map.class);
