...
public class PageBisQyjbxxController extends BaseController {
    ...
    @ResponseBody
    public Map<String, Object> getMarker(HttpServletRequest request) {
        Map<String, Object> mapdata = getAuthorityMap();
        String qyname = request.getParameter("qyname");
        List<Map<String, Object>> list = bisQyjbxxService.getMarker(qyname);
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("data", list);
        return map;
    }
    ...
    @ResponseBody
    public boolean checkLoginName(String qyName, Long qyid) {
        return bisQyjbxxService.checkQyname(qyName,qyid);
    }
     ...
     * 可视化数据 - 应急大数据
     */
    @RequestMapping(value="onemap2")
    public String oneMap2(HttpServletRequest request,Model model) {
        Barrio bro=barrioservice.findPointBycode(UserUtil.getCurrentShiroUser().getXzqy());
        String type=request.getParameter("type");
        if(StringUtils.isNoneBlank(type)){
            model.addAttribute("disablescroll", "yes");
        }
        if(bro!=null)
            model.addAttribute("mappoint", bro.getMappoint());
        return "qyxx/qyjbxx/mapindex2";
    }
