...
public class PageWghglJdkhController extends BaseController {
	 ...
	 * @param request
	 */
	@RequestMapping(value="yjlist2")
	@ResponseBody
	public String getyjData2(HttpServletRequest request, Model model) {
		Map<String, Object> map = getPageMap(request);
		String nf="";
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy");
		Date date = new Date();
		...
		map.put("xzqy",xzqy);//行政区域
		map.putAll(getAuthorityMap());
		return JsonMapper.getInstance().toJson(wghglJdkhService.yjdataGrid2(map));
	}
	 ...
	 * @param request
	 */
	@RequestMapping(value="wgfxdph")
	@ResponseBody
	public String wgfxdph(HttpServletRequest request, Model model) {
		Map<String, Object> map = getPageMap(request);
		String xzqy =UserUtil.getCurrentShiroUser().getXzqy();
		map.put("xzqy",xzqy);//行政区域
		return JsonMapper.getInstance().toJson(wghglJdkhService.wgfxdph(map));
	}
