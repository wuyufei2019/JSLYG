...
public class PageAqpxZxksController extends BaseController {
    ...
    @ResponseBody
    public String isOrNotCreateSJ(@PathVariable("id") Long kcid,
        ...
        if (stgz == null) {
            ...
            long diff = (start == null ? 0 : System.currentTimeMillis() - start);
            //获取学习时长 秒
            Integer totalTime = aqpxXxjlService.getTotalTime(Long.valueOf(sessionuser.getId()), kcid);
            AQPX_CourseEntity course = aqpxKCxxService.findbyid(kcid);
            Float m2 = Float.valueOf(course.getM2());
            if (diff + totalTime * 1000 < m2 * 60 * 60 * 1000)
                datasuccess =  "学时不足，请继续学习！";
        }
        return datasuccess;
    }
