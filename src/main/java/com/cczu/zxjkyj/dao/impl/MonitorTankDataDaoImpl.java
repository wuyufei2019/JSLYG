package com.cczu.zxjkyj.dao.impl;

import com.cczu.util.dao.BaseDao;
import com.cczu.zxjkyj.dao.IMonitorTankDataDao;
import com.cczu.zxjkyj.entity.Main_SignalStorageEntity;
...
public class MonitorTankDataDaoImpl extends BaseDao<Main_SignalStorageEntity, Long> implements IMonitorTankDataDao {
	...
	public List<Map<String, Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql=" SELECT TOP "+mapData.get("pageSize")+" * FROM ("+
				"select ROW_NUMBER() OVER (ORDER BY bis.m1,tank.id) AS RowNumber,tank.id,bis.id qyid,bis.m1 qyname, " +
				"tank.m1 cgname,tank.m2 lx,tank.m3 rj,tank.m7 wl,tank.m9 wh,tank.m10 gj,tank.m11 gh,sig.level yw,sig.reserves cl,sig.temperature wd,sig.pressure yl,sig.[percent] bl,sig.colltime sj FROM (SELECT * FROM main_signal_storage t1 WHERE id =(SELECT max(id) FROM main_signal_storage t2 where t2.ID1=t1.id1  )) sig LEFT JOIN bis_tank tank on tank.ID=sig.id1 LEFT JOIN bis_enterprise bis on  bis.id=tank.ID1 where 1=1"+content+
	...
	public List<Map<String, Object>> findInfoById(long id) {
		String sql="SELECT tank.m2,tank.m7,tank.m9,tank.m11,tank.m8,sig.* FROM bis_tank tank  INNER JOIN (SELECT * " +
				"FROM main_signal_storage t1 WHERE id =(SELECT max(id) FROM main_signal_storage t2 where t2.ID1=t1.id1   )) sig on tank.ID=sig.id1 WHERE tank.ID1= '"+id+"'" ;
		List<Map<String,Object>> list=findBySql(sql,null,Map.class);
		return list;
	}
	...
	public List<Map<String, Object>> getMatLsbdDate(Map<String, Object> mapData) {
		// TODO Auto-generated method stub
		String content=content2(mapData);
		String sql="SELECT ct,dic.label,round(sum(avg),2) count from (SELECT a.id1,avg(a.reserves) avg,CONVERT(VARCHAR(10),a.colltime,120) ct from main_signal_storage a where a.colltime>='"+mapData.get("datestart")+" 00:00:00.000' and a.colltime<='"+mapData.get("dateend")+" 23:59:59.999'  GROUP BY a.id1,CONVERT(VARCHAR(10),a.colltime,120))a LEFT JOIN bis_tank tank  on tank.ID=a.id1 left join bis_enterprise bis on bis.id=tank.id1  LEFT JOIN t_dict dic on dic.value= tank.id3 and type='wxhxpfl' where 1=1 "+content+" GROUP BY ct,dic.LABEL ORDER BY ct desc";
		List<Map<String, Object>> list = findBySql(sql,null,Map.class);
		return list;
	}
