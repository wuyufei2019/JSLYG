...
public class wordToPDF {
    ...
    public static void WordToPDF(String docfile, String pdf2File) {
        ...
        if (sourceFile.exists()) {
            if (!pdfFile.exists()) {
                OpenOfficeConnection connection = new SocketOpenOfficeConnection(Integer.valueOf(Global.getConfig("openoffice_port")));
                try {
                    connection.connect();
...
//		System.out.println(purl);
     ...
     * @param filePath
     * @param oldfilename
     * @param filename    引入下面 jacob-1.14.3-x64 依赖, 将jacob-1.14.3-x64.dll 放到 jre/bin、System32文件夹、System64文件夹中中下
