...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
    ...
    public Page<T> findPageSql(final Page<T> page,Class<?> resultClass ,final String sql, final Object... values) {
        Assert.notNull(page, "page不能为空");
        ...
        ((SQLQuery) q).addEntity(resultClass);
        setPageParameterToQuery(q, page);
        List result = q.list();
        page.setResult(result);
        return page;
    ...
    protected int countSqlResult(final String sql, final Object... values) {
        ...
        try {
            Integer count = (int) createSQLQuery(countSql, values).uniqueResult();
            return count;
        } catch (Exception e) {
    ...
    public Page<Map> findMapPageSql(final Page<Map> page, final String sql, final Object... values) {
        Assert.notNull(page, "page不能为空");
